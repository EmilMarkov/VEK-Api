...
pub fn kaoskrew_formatter(title: String) -> String {
    let formatted = Regex::new(r"(v\.?[0-9])+([0-9]|\.)+")
        .unwrap()
        .replace_all(&title, "");
    let formatted =
        Regex::new(r"(\.Build\.[0-9]*)?(\.MULTi[0-9]{1,2})?(\.REPACK-KaOs|\.UPDATE-KaOs)?")
            .unwrap()
...
pub fn xatab_formatter(title: String) -> String {
    let formatted = title
        ...
        .replace("R.G. GOGFAN", "");
    let formatted = Regex::new(r"[\u0400-\u04FF]")
        .unwrap()
        .replace_all(&formatted, "");
    Regex::new(r"(v\.?([0-9]| )+)+([0-9]|\.|-|_|/|[a-zA-Z]| )+")
        .unwrap()
        .replace_all(&formatted, "")
        .to_string()
}
...
pub fn gog_formatter(title: String) -> String {
    Regex::new(r"(v\.[0-9]+|v[0-9]+\.|v[0-9]{4})+.+")
        .unwrap()
        .replace_all(&title, "")
        .to_string()
}
